...
func reduceFunc(input []mapreduce.KeyValue) (result []mapreduce.KeyValue) {
	...
	var(
		mapAux map[string]int = make(map[string]int)
		tmp			  mapreduce.KeyValue
	) 

	for _, item := range input {
		_, hasKey := mapAux[item.Key]

		if !hasKey {
			mapAux[item.Key] = 0
		}

		s, _ := strconv.Atoi(item.Value)
		mapAux[item.Key] += s
	}

	/* Clear the result map before filling it with the correct data. */
	result = make([]mapreduce.KeyValue, 0)

	for key, value := range mapAux {
		tmp.Key = key
		tmp.Value = strconv.Itoa(value)
